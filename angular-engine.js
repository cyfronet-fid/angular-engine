...
angular.module('engine.document').factory('DocumentFieldFactory', ["DocumentField", "$engine", "$engLog", "createAttachmentCtrl", function (DocumentField, $engine, $engLog, createAttachmentCtrl) {
...
}]).factory('DocumentField', ["ConditionBuilder", "$engLog", "$q", "$rootScope", "$engine", function (ConditionBuilder, $engLog, $q, $rootScope, $engine) {
    ...
    DocumentField.prototype.makeField = function makeField(metricList, metric, ctx) {
        ...
        formlyField.data.onChangeHandlers.push(function (value, model, scope) {
            return $engine.ON_METRIC_CHANGE(model.data.metric.id, scope.originalModel, DocumentField.allFields);
        });
        ...
        link: function link(scope, element, attrs, ngModelCtrl) {
            ...
            scope.$watch('model', function (val, old) {
                if (typeof val == 'string') {
                    if (!val.match(/^-?\d+$/))
                        // scope.model = val;
                        scope.model = old;else {
                        var pv = Number(val);
                        if (!_.isNaN(pv) && pv < JAVA_MAX_INT && pv > JAVA_MIN_INT) scope.model = pv;else scope.model = val;
                    }
                }
            });
        }
                ...
                if (typeof val == 'string') {
                    switch (scope.options.data.metric.inputType) {
                        case 'INTEGER':
                            {
                                if (val !== '' && !val.match(/^-?\d*$/)) {
                                    ...
                                    var nv = (ngModelCtrl.$modelValue || '').toString();
                                    ...
                                    var pv = Number(val);
                                    if (!_.isNaN(pv)) {
                                        if (pv > JAVA_MIN_INT && pv < JAVA_MAX_INT) return pv;else {
                                            var correctedVal = pv < 0 ? JAVA_MIN_INT : JAVA_MAX_INT;
                        ...
                        case 'FLOAT':
                            {
                                var _pv = Number(val);
                                if (!_.isNaN(_pv)) return _pv;else return val;
                                break;
                            }
                        default:
                            return val;
        ...
        return engineMetric(options, function (metricList) {
            ...
            _.forEach(self.metricList, function (metric) {
                if (!(metric.id in metricDict)) {
                    ...
                    var fieldIndex = self.fieldList.findIndex(function (field) {
                        return field.key === metric.id;
                    });
                    if (fieldIndex !== -1) self.fieldList.splice(fieldIndex, 1);
...
.provider('$engine', ["$routeProvider", "$engineApiCheckProvider", "$engineFormlyProvider", "$injector", function ($routeProvider, $engineApiCheckProvider, $engineFormlyProvider, $injector) {
    ...
    var GLOBAL_CSS = '';
    var ON_METRIC_CHANGE = function ON_METRIC_CHANGE(metricId, document, metrics) {};
     ...
     *
     */
    this.setOnMetricChange = function (handler) {
...
var ENGINE_COMPILATION_DATE = '2019-02-26T19:31:10.148Z';
var ENGINE_VERSION = '0.8.21';
...
app.controller('engineListCtrl', ["$scope", "$route", "$location", "engineMetric", "$engine", "engineQuery", "engineAction", "engineActionsAvailable", "engineActionUtils", "engineResolve", "DocumentModal", "$engLog", "DocumentActionList", "$timeout", "DocumentAction", "$injector", "$rootScope", "$parse", "$controller", function ($scope, $route, $location, engineMetric, $engine, engineQuery, engineAction, engineActionsAvailable, engineActionUtils, engineResolve, DocumentModal, $engLog, DocumentActionList, $timeout, DocumentAction, $injector, $rootScope, $parse, $controller) {
    ...
    this.loadDocuments = function (clear) {
        ...
        if (this.parentDocument == null || this.parentDocument != null && this.parentDocument.id != null) {
            ...
            $scope.documents.$promise.then(function (documents) {
                ...
                if (self.formlyOptions.templateOptions.serverErrors.length != 0) self.formlyOptions.data.form.validateCurrentStep(true);
            });
        } else {
    ...
    $scope.genDocumentLink = function (documentId, hash, column, document) {
        var idExpression = documentId || $parse(column.id || 'id')(document);
...
angular.module("engine").run(["$templateCache", function ($templateCache) {
  $templateCache.put("/src/formly/types/templates/input.tpl.html", "<input class=\"form-control\"  ng-model=\"model[options.key]\" placeholder=\"{{options.templateOptions.placeholder | translate}}\" number-format ng-keypress=\"onkeypress($event)\">");
}]);
angular.module("engine").run(["$templateCache", function ($templateCache) {
...
angular.module("engine").run(["$templateCache", function ($templateCache) {
  $templateCache.put("/src/formly/types/templates/select.tpl.html", "<select class=\"form-control\" ng-model=\"model[options.key]\" ng-options=\"option[to.valueProp || 'value'] as option[to.labelProp || 'name'] | translate for option in to.options\"></select>");
}]);
angular.module("engine").run(["$templateCache", function ($templateCache) {
...
angular.module("engine").run(["$templateCache", function ($templateCache) {
  $templateCache.put("/src/list/cell/link.tpl.html", "<a ng-href=\"{{::genDocumentLink(null, true, column, document_entry.document)}}\" ng-click=\"onDocumentSelect(document_entry)\" class=\"proposal-title\" ng-include=\"getCellTemplate(document_entry.document, column, true)\"></a>");
}]);
angular.module("engine").run(["$templateCache", function ($templateCache) {
