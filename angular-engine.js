...
angular.module('engine.document').component('engineDocumentDetails', {
    ...
    controller: ["$parse", "$filter", function controller($parse, $filter) {
        var _this = this;
...
angular.module('engine.document').controller('engAttachmentCtrl', ["$scope", "Upload", "$engine", "$timeout", "engAttachment", "$engLog", function ($scope, Upload, $engine, $timeout, engAttachment, $engLog) {
    var self = this;
...
var ENGINE_COMPILATION_DATE = '2017-10-20T18:19:12.825Z';
var ENGINE_VERSION = '0.7.12';
...
app.controller('engineListCtrl', ["$scope", "$route", "$location", "engineMetric", "$engine", "engineQuery", "engineAction", "engineActionsAvailable", "engineActionUtils", "engineResolve", "DocumentModal", "$engLog", "DocumentActionList", "$timeout", "$injector", "$rootScope", "$parse", "$controller", function ($scope, $route, $location, engineMetric, $engine, engineQuery, engineAction, engineActionsAvailable, engineActionUtils, engineResolve, DocumentModal, $engLog, DocumentActionList, $timeout, $injector, $rootScope, $parse, $controller) {
    ...
    this.canShowPagination = function () {
        if ($scope.documents.length < this.DOCUMENT_QUERY_LIMIT && this.documentPages.length <= 1) return false;

        if (this.noParentDocument === true) return false;

        if ($scope.documents.$error != null) return false;

        if (!$scope.documents.$resolved) return false;
    ...
    this.loadDocuments = function (clear) {
        ...
        filters = _.isEmpty(filters) ? null : { $and: _.map(filters, function (val, key) {
                return _defineProperty({}, key, { $regex: val, $options: 'i' });
            }) };
