...
angular.module('engine.document').component('engineDocumentDetails', {
    ...
    controller: ["$parse", function controller($parse) {
        var self = this;
        this.$parse = $parse;
...
app.controller('engineDocumentCtrl', ["$scope", "$route", "engineMetric", "$routeParams", "$engine", "engineDocument", "engineActionsAvailable", "$location", "engineActionUtils", "DocumentEventCtx", "engineAction", "engineMetricCategories", "StepList", "DocumentForm", "DocumentActionList", "$q", "$engLog", "$attrs", "Step", "$parse", function ($scope, $route, engineMetric, $routeParams, $engine, engineDocument, engineActionsAvailable, $location, engineActionUtils, DocumentEventCtx, engineAction, engineMetricCategories, StepList, DocumentForm, DocumentActionList, $q, $engLog, $attrs, Step, $parse) {
    ...
    this.$onInit = function () {
        ...
        if ($parse($attrs['documentScope']).assign === true) self.documentScope = $scope;
...
angular.module('engine.document').factory('DocumentModal', ["$resource", "$uibModal", "$translate", "$timeout", function ($resource, $uibModal, $translate, $timeout) {
...
angular.module('engine.document').controller('engineDocumentWrapperCtrl', ["$scope", "$route", "$location", "engineMetric", "$routeParams", "engineResolve", "StepList", function ($scope, $route, $location, engineMetric, $routeParams, engineResolve, StepList) {
    $scope.engineResolve = engineResolve;
    $scope.options = $route.current.$$route.options;
...
}]).factory('DocumentAction', ["engActionResource", "$engineApiCheck", "DocumentActionProcess", "$engLog", "$q", "$rootScope", function (engActionResource, $engineApiCheck, DocumentActionProcess, $engLog, $q, $rootScope) {
...
}]).factory('DocumentActionProcess', ["$location", "$engine", "engineDocument", "$engLog", "$q", function ($location, $engine, engineDocument, $engLog, $q) {
...
angular.module('engine.document').controller('engAttachmentCtrl', ["$scope", "Upload", "$timeout", "engAttachment", "$engLog", function ($scope, Upload, $timeout, engAttachment, $engLog) {
    var self = this;
...
}]).factory('DocumentField', ["ConditionBuilder", function (ConditionBuilder) {
 ...
 */
.provider('$engine', ["$routeProvider", "$engineApiCheckProvider", "$engineFormlyProvider", function ($routeProvider, $engineApiCheckProvider, $engineFormlyProvider) {
    var self = this;
     ...
     *
     */
    this.$get = ["$engineFormly", "engineDocument", "$rootScope", "$engLog", "engineQuery", function ($engineFormly, engineDocument, $rootScope, $engLog, engineQuery) {
...
angular.module('engine').factory('engineResolve', function () {
    ...
    return engResource;
}]).service('engineQuery', ["$engineConfig", "$engineApiCheck", "$http", "EngineInterceptor", "$q", function ($engineConfig, $engineApiCheck, $http, EngineInterceptor, $q) {
    ...
    return {
        ...
        get: function get(query, parentDocument, callback, errorCallback, skip, limit, sort, filters) {
            $engineApiCheck.throw([$engineApiCheck.string, $engineApiCheck.object.optional, $engineApiCheck.func.optional, $engineApiCheck.func.optional, $engineApiCheck.number.optional, $engineApiCheck.number.optional, $engineApiCheck.array.optional, $engineApiCheck.object.optional], arguments);
...
var ENGINE_COMPILATION_DATE = '2017-08-30T17:14:56.199Z';
var ENGINE_VERSION = '0.7.5';
...
app.controller('engineListCtrl', ["$scope", "$route", "$location", "engineMetric", "$engine", "engineQuery", "engineAction", "engineActionsAvailable", "engineActionUtils", "engineResolve", "DocumentModal", "$engLog", "DocumentActionList", "$timeout", "$injector", "$rootScope", "$parse", "$controller", function ($scope, $route, $location, engineMetric, $engine, engineQuery, engineAction, engineActionsAvailable, engineActionUtils, engineResolve, DocumentModal, $engLog, DocumentActionList, $timeout, $injector, $rootScope, $parse, $controller) {
    var self = this;
    ...
    this.loadDocuments = function () {
        ...
        if (this.parentDocument == null || this.parentDocument != null && this.parentDocument.id != null) {
            $scope.documents = engineQuery.get($scope.query, this.parentDocument, undefined, undefined, self.documentPages.length * self.DOCUMENT_QUERY_LIMIT, self.DOCUMENT_QUERY_LIMIT, this.ordering, this.filters);
            $scope.documents.$promise.then(function (documents) {
                ...
                if ($scope.documents.length !== self.DOCUMENT_QUERY_LIMIT) this.allDocumentsLoaded = true;
...
angular.module('engine.list').controller('engineListWrapperCtrl', ["$scope", "$route", "engineDashboard", function ($scope, $route, engineDashboard) {
    $scope.options = $route.current.$$route.options;
